...
namespace qt_test
{
    ...
    void qt_node::icp_go(){
        ...
        c_point T2 = c_point(DOCK_X -0.05, DOCK_Y + 0.16248);
        c_point T4 = c_point(T2.first , T2.second+0.03);
        c_point T6 = c_point(T4.first+0.18, T4.second);

        c_point T3 = c_point(DOCK_X -0.05, DOCK_Y - 0.16248);
        c_point T5 = c_point(T3.first, T3.second-0.03);
        c_point T7 = c_point(T5.first+0.18, T5.second);

        KFRAME frm0 = generateSampleKFrame(T1,T2, T3,T4,T5,T6,T7);
        KFRAME frm1 = generateVKFrame(); //target
        ...
        double err2 = kfrm_icp(frm0, frm1, dock_tf);
        RCLCPP_INFO(get_logger(), "ICP error: %f", err2);
        RCLCPP_INFO(get_logger(), "dock_tf: \n%f %f %f %f\n%f %f %f %f\n%f %f %f %f\n%f %f %f %f", dock_tf(0,0), dock_tf(0,1), dock_tf(0,2), dock_tf(0,3), dock_tf(1,0), dock_tf(1,1), dock_tf(1,2), dock_tf(1,3), dock_tf(2,0), dock_tf(2,1), dock_tf(2,2), dock_tf(2,3), dock_tf(3,0), dock_tf(3,1), dock_tf(3,2), dock_tf(3,3));
        RCLCPP_INFO(get_logger() , "degree error is %f", atan2(dock_tf(1,0),dock_tf(0,0)));
    }

    void qt_node::timerCallback() {


        // 예상 Lidar 샘플 KFRAME 퍼블리시
        c_point T1 = c_point(DOCK_X, 0);
        ...
        KFRAME kframe = generateSampleKFrame(T1, T2, T3,T4,T5,T6,T7);
        publishKFrameMarker(kframe, 0, "sample_kframe", 1.0, 0.0, 0.0);
        KFRAME frm1 = generateVKFrame(); //target
        publishKFrameMarker(frm1, 1, "v_kframe", 0.0, 1.0, 0.0);
...
KFRAME qt_node::generateSampleKFrame(const c_point& p1, const c_point& p2, const c_point& p3 , const c_point&p4,const c_point&p5,const c_point&p6,const c_point&p7)
{
    // Gaussian noise generator
    std::default_random_engine generator;
    std::normal_distribution<double> distribution(0.0, sample_noise);
    ...
    XYZR_CLOUD res6;
    XYZR_CLOUD res7;
    XYZR_CLOUD left_cloud;
    XYZR_CLOUD right_cloud;
    res1 = generateSamplePoints (p1,p2,30);
    res2 = generateSamplePoints (p2,p4,8);
    res3 = generateSamplePoints (p4,p6,30);
    
    res4 = generateSamplePoints (p1,p3,30);
    res5 = generateSamplePoints (p3,p5,8);

    left_cloud.pts.insert(left_cloud.pts.end(), res1.pts.begin(), res1.pts.end());
    left_cloud.pts.insert(left_cloud.pts.end(), res2.pts.begin(), res2.pts.end());
    left_cloud.pts.insert(left_cloud.pts.end(), res3.pts.begin(), res3.pts.end());

    right_cloud.pts.insert(right_cloud.pts.end(), res4.pts.begin(), res4.pts.end());
    right_cloud.pts.insert(right_cloud.pts.end(), res5.pts.begin(), res5.pts.end());
...
KFRAME qt_node::generateVKFrame()
{
    ...
    //p1 is middle of the v marker
        ...
        c_point p1 = c_point(ROBOT_SIZE_X[1]+ 0.09,0.0);
        ...
        // res_l = generateSamplePoints(p1, p2, 30);
        // res_r = generateSamplePoints(p1, p3, 30);
        // res1 = generateSamplePoints(p6,p4,30);
        res2 = generateSamplePoints(p4,p2,15);
        res3 = generateSamplePoints(p2,p1,30);
        res4 = generateSamplePoints(p1,p3,30);
        res5 = generateSamplePoints(p3,p5,15);
        // res6 = generateSamplePoints(p5,p7,30);
