...
void MainWindow::onTimer(){
        ...
        c_point T2 = c_point(node->DOCK_X -0.05, node->DOCK_Y +0.16248);
        c_point T4 = c_point(T2.first , T2.second+0.03);
        c_point T6 = c_point(T4.first+0.18, T4.second);

        c_point T3 = c_point(node->DOCK_X -0.05, node->DOCK_Y - 0.16248);
        c_point T5 = c_point(T3.first, T3.second-0.03);
        c_point T7 = c_point(T5.first+0.18, T5.second);
        
    node->publishKFrameMarker(node->generateSampleKFrame(T1,T2,T3,T4,T5,T6,T7), 0, "current_lidar", 1.0, 0.0, 0.0);
